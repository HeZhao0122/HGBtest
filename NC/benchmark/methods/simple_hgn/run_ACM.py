...
def run_model_ACM(args):
    ...
    _, _, features_list, adjM, type_mask, labels, train_val_test_idx, dl = load_ACM_data()
    ...
    for iter in range(args.repeat):
        ...
        early_stopping = EarlyStopping(patience=args.patience, verbose=True,
                                       save_path='checkpoint/checkpoint_{}_{}.pt'.format("ACM", iter))
        ...
        net.load_state_dict(torch.load('checkpoint/checkpoint_{}_{}.pt'.format("ACM", iter)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            logits = net(new_adjM, x)
            ...
            dl.gen_file_for_evaluate(test_idx=test_idx, label=pred, file_path="{}_{}.txt".format("ACM", iter+1))
            pred = onehot[pred]
