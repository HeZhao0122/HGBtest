...
def run_model_DBLP(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    for iter in range(args.repeat):
        ...
        early_stopping = EarlyStopping(patience=args.patience, verbose=True,
                                       save_path='checkpoint/checkpoint_{}_{}.pt'.format("DBLP", iter))
        ...
        net.load_state_dict(torch.load('checkpoint/checkpoint_{}_{}.pt'.format("DBLP", iter)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            logits = net(new_adjM, x)
            ...
            dl.gen_file_for_evaluate(test_idx=test_idx, label=pred, file_path="{}_{}.txt".format("DBLP", iter + 1))
            pred = onehot[pred]
