...
def run_model_IMDB(feats_type, num_layers, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    for iter in range(repeat):
        ...
        early_stopping = EarlyStopping(patience=args.patience, verbose=True,
                                       save_path='checkpoint/checkpoint_{}_{}.pt'.format("IMDB", iter))
        dur1 = []
        dur2 = []
        dur3 = []
        ...
        net.load_state_dict(torch.load('checkpoint/checkpoint_{}_{}.pt'.format("IMDB", iter)))
        net.eval()
        with torch.no_grad():
            logits = net(new_adjM, x)
            ...
            dl.gen_file_for_evaluate(test_idx=test_idx, label=pred, file_path="{}_{}.txt".format("IMDB", iter + 1))
            pred = onehot[pred]
