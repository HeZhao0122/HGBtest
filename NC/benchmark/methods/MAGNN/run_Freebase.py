...
for fn in ['meta1.json', 'meta2.json']:
  with open(fn, 'r', encoding='utf-8') as f:
    ...
    for x in meta['node_0'][:2]:
        tmp = list(map(int, x['path'].split(',')))
        tmp = tmp + [-x-1 for x in reversed(tmp)]
        etypes_list.append([x if x>=0 else x+36 for x in tmp])
...
def run_model_DBLP(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    adjlists, edge_metapath_indices_list, features_list, adjM, type_mask, labels, train_val_test_idx, dl = load_Freebase_data()
    ...
    for iter in range(repeat):
        net = MAGNN_nc_mb(len(etypes_list), 72, etypes_list, in_dims, hidden_dim, out_dim, num_heads, attn_vec_dim, rnn_type, dropout_rate)
        net.to(device)
        ...
        with torch.no_grad():
            ...
            dl.gen_file_for_evaluate(test_idx, pred, 'Freebase_{}.txt'.format(iter))
...
if __name__ == '__main__':
    ...
    ap.add_argument('--save-postfix', default='Freebase', help='Postfix for the saved model and result. Default is DBLP.')
