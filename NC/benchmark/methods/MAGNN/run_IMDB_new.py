...
def run_model_IMDB(feats_type, num_layers, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    nx_G_lists, edge_metapath_indices_lists, features_list, adjM, type_mask, labels, train_val_test_idx, dl = å()
    ...
    for iter in range(repeat):
        ...
        with torch.no_grad():
            ...
            pred = logp[test_idx].cpu().numpy()
            dl.gen_file_for_evaluate(test_idx, pred, 'IMDB_{}.txt'.format(iter))
