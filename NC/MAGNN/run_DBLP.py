...
from utils.tools import index_generator, evaluate_results_nc, parse_minibatch, pairwise_loss, creat_pairs, get_simM, sim_loss
...
def run_model_DBLP(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
                   num_epochs, patience, batch_size, neighbor_samples, repeat, save_postfix, use_ploss):
    ...
    for _ in range(repeat):
        ...
        for epoch in range(num_epochs):
            ...
            for iteration in range(train_idx_generator.num_iterations()):
                ...
                ce_loss = F.nll_loss(logp, labels[train_idx_batch])
                train_loss = ce_loss

                loss_sim = sim_loss(simM, embeddings, train_idx_batch)
                train_loss += loss_sim
                t2 = time.time()
                dur2.append(t2 - t1)
            ...
            with torch.no_grad():
                for iteration in range(val_idx_generator.num_iterations()):
                    ...
                    loss_sim = sim_loss(simM, embeddings, val_idx_batch)
                    val_sim_loss.append(loss_sim)
                if use_ploss:
                    val_s_loss = sum(val_sim_loss)/len(val_sim_loss)
                    val_loss = F.nll_loss(torch.cat(val_logp, 0), labels[val_idx]) + val_s_loss
                else:
...
if __name__ == '__main__':
    ...
    ap.add_argument('--use-ploss', default=False, help='Use pairwise loss')
    args = ap.parse_args()
    run_model_DBLP(args.feats_type, args.hidden_dim, args.num_heads, args.attn_vec_dim, args.rnn_type,
                   args.epoch, args.patience, args.batch_size, args.samples, args.repeat, args.save_postfix, args.use_ploss)
